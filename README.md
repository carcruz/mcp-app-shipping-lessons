...
- Container-build gotchas for an MCP App server: build-context location in a monorepo, stale/missing client bundles, platform mismatches, and runtime-read files (config/templates) the Dockerfile forgot to `COPY`
...
- Diagnosing a generic "Unable to reach `<server>`" host error — checking the client-side transport log to tell a real connectivity failure apart from an application-level error the host's generic messaging is masking
- Why an MCP server with in-memory session state needs a `min-instances` floor on autoscaled/serverless platforms (Cloud Run and similarly-shaped targets) — scale-to-zero silently wipes sessions and looks like a random, intermittent connectivity bug
